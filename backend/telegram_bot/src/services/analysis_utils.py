...
def import_analysis_functions():
    ...
    try:
        ...
        from ..bot.handlers.callback_handlers import analysis_service as _service, analyze_with_smc as _analyze, format_price as _format
        analysis_service = _service
        analyze_with_smc = _analyze
        format_price = _format
        logger.info("Successfully imported analysis functions using relative import")
        return True
        
    except ImportError:
        try:
            # Method 2: Add handlers to sys.path
            ...
            logger.info("Successfully imported analysis functions using sys.path")
            return True
            
        except ImportError:
            try:
                # Method 3: Alternative path resolution
                ...
                analyze_with_smc = callback_handlers.analyze_with_smc
                format_price = callback_handlers.format_price
                logger.info("Successfully imported analysis functions using alternative path")
                return True
