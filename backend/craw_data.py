...
def get_binance_usdt_pairs():
    """Lấy danh sách các cặp /USDT trên Binance."""
    try:
...
def calculate_indicators(df_display, df_calc):
    ...
    # Chuyển đổi timestamp sang giây (Unix timestamp)
    df_display_reset = df_display.reset_index()
    df_display_reset['time'] = df_display_reset['timestamp'].astype('int64') // 10 ** 9

    # Dữ liệu nến
    ohlc_data = df_display_reset[['time', 'open', 'high', 'low', 'close']].to_dict(orient='records')

    # Dữ liệu volume với màu sắc
    df_display_reset['color'] = ['rgba(0, 150, 136, 0.8)' if row['close'] >= row['open'] else 'rgba(255, 82, 82, 0.8)'
                                 for _, row in df_display_reset.iterrows()]
    volume_data = df_display_reset[['time', 'volume', 'color']].rename(columns={'volume': 'value'}).to_dict(
        orient='records')

    # Dữ liệu FVG
    candle_duration_sec = (df_calc.index[1] - df_calc.index[0]).total_seconds() if len(df_calc.index) > 1 else 3600
    last_time_sec = df_display_reset['time'].iloc[-1]

    fvgs = []
    bullish_fvgs = df_calc[df_calc['is_bullish_fvg']]
    for i, fvg in bullish_fvgs.iterrows():
        start_time = (i.value // 10 ** 9) + candle_duration_sec
        fvgs.append({'startTime': start_time, 'endTime': last_time_sec, 'bottom': fvg['bullish_fvg_bottom'],
                     'top': fvg['bullish_fvg_top'], 'color': 'rgba(0, 255, 0, 0.15)'})

    bearish_fvgs = df_calc[df_calc['is_bearish_fvg']]
    for i, fvg in bearish_fvgs.iterrows():
        start_time = (i.value // 10 ** 9) + candle_duration_sec
        fvgs.append({'startTime': start_time, 'endTime': last_time_sec, 'bottom': fvg['bearish_fvg_bottom'],
                     'top': fvg['bearish_fvg_top'], 'color': 'rgba(255, 0, 0, 0.15)'})

    # Dữ liệu BOS/CHoCH
    formatted_breaks = [
        {'price': b['price'], 'startTime': b['start_idx'].value // 10 ** 9, 'endTime': b['end_idx'].value // 10 ** 9,
         'type': b['type'],
         'color': 'green' if 'BOS' in b['type'] and b['color'] == 'lime' else 'red' if 'BOS' in b['type'] else 'orange'}
        for b in breaks]

    # Dữ liệu Fibonacci
    if fibo_levels:
        fibo_start_time = min(sh_idx.value, sl_idx.value) // 10 ** 9
...
def calculate_market_structure(df):
    """Hàm tính toán cấu trúc thị trường (giữ nguyên)."""
    ...
    for i in range(1, len(df)):
        ...
        is_high_broken, is_low_broken = current_candle['close'] > structure_high, current_candle[
            'close'] < structure_low
        if direction in [0, 2] and is_high_broken:
            break_type = 'BOS' if direction == 2 else 'CHoCH'
            breaks.append(
                {'price': structure_high, 'start_idx': sh_idx, 'end_idx': current_candle.name, 'type': break_type,
                 'color': 'lime'})
            direction, structure_high, sh_idx, structure_low, sl_idx = 2, current_candle[
            ...
            breaks.append(
                {'price': structure_low, 'start_idx': sl_idx, 'end_idx': current_candle.name, 'type': break_type,
                 'color': 'red'})
            direction, structure_low, sl_idx, structure_high, sh_idx = 1, current_candle[
